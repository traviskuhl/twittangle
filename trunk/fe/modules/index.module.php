...
class index extends Tangle {
			...
			if ( $row AND is_array($data) AND $c['ip'] == $_SERVER['REMOTE_ADDR'] AND $c['sig'] == md5($row['data']) ) {
				...
				if ( $resp ) {				
					...
					if ( $db ) {
						...
						// save session cookie
						setcookie($this->ttCookie,$session,$ttl,'/',DOMAIN);
						setcookie("L",time(),$ttl,'/',DOMAIN);
						setcookie($this->ttaCookie,$this->md5($resp['id']),$ttl,'/',DOMAIN);
	...
	public function home() {
		...
		$menu_messages = array(
			...
			'dm'		=> array( 'Direct Messages Received', 'direct_messages.json?page='.$page ),			
			'dmsent'	=> array( 'Direct Messages Sent', 'direct_messages/sent.json?page='.$page ),
		...
		if ( $loadnow ) {	
			...
			if ( $path[0] == 'timeline' ) {
			...
			else if ( $path[0] == 'fav-group' ) {
			
				// get it 
				$group = $this->user['favGroups'][$path[1]];
				...
				$title = 'Favorite Group / ' . $group['name'];
	...
	public function login() {
		...
		if ( isset($path[0]) AND $path[0] == 'switch' ) {
			...
			if ( $user['oauth'] ) {
				...
				setcookie($this->ttCookie,$session,$ttl,'/',DOMAIN);
				setcookie("L",time(),$ttl,'/',DOMAIN);
				setcookie($this->ttaCookie,$this->md5($user['id']),$ttl,'/',DOMAIN);			
	...
	protected function remove() {
		...
		echo "
			...
			<div class='yui-gc'>
				...
				<div class='yui-u'>
					<div class='module'>
						<div class='hd'><h2>No Authorization Necessary</h2></div>
						<div class='bd'>
							<p class='padd10'>If you don't want to use the authorization method to remove yourself from twitTangle, just tweet:<br> <span class='b'>@twittangle remove!</span></p> 
							
							<a class='login-btn ignore' style='width: 250px; margin: 20px auto 0;' href='http://twitter.com/?status=@twittangle%20remove!'>Tweet it Now!</a>												
							
							<p class='gray padd10 center small'>The tweet must be in the exact format: \"@twittangle remove!\" (without the quotes). It may take up to 24 hours for your profile to be fully removed.</p>
						</div>
					</div>					
				</div>
			</div>
