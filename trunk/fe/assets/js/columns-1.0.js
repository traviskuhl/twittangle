...
(function(){
	...
	$Y.twitTangle.Columns.prototype = {
		...
		mouse : function(e,type) {
			...
			if ( $d.hasClass(tar,'column-right-arrow') ) {
				...
				if ( type == 'out' ) {
					...
					this.flags.scrollL = window.setInterval(function(){
						$d.setX('page-content',$d.getX('page-content')+10)
					},50);
				}
			}
			...
			else if ( (tar = $d.isGoodTarget(oTar,'side-menu')) && tar.tagName.toLowerCase() == 'ul' ) {
				if ( type == 'out' ) {
		...
		setup : function() {	
			...
			// move the menu there
			var a = new $a('side-menu',{ 'top': {'to': 20 }, 'left': {'to': 50 }},.5);
				a.animate();
			...
			var a = new $a('page-content',{'opacity':{'to':0}},.5);
				a.onComplete.subscribe(function(){
					...
					$d.setStyle('side-menu','width', 'auto' );		
