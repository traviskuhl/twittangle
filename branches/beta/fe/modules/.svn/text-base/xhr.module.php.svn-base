	...
	public function notify() {
		$html = "
			<div class='notify-panel'>
				<h2>Hello {$this->user['info']['name']}.</h2> <p>Sorry for the interruption, but we wanted to tell you about some of the newest features at twitTangle. So here's a quick list...</p>
			
				<ul>
					<li> <em>New Design:</em> We have a new look. Alright it's not completely new, but hopefully the changes make the site a little easier to use.  </li>
					<li> <em>Group Settings:</em> Ever wanted to share one of you Groups with friends. Now you can! You can now share your Groups with friends or make your Group completely public. <a class='ignore' href='".$this->url('my-groups')."'>Check it out!</a> </li>
					<li> <em>Column View:</em> Having to click through all your different groups and timelines is a pain. Our new Column View lets you open multiple timelines on a single page. <a class='ignore' href='".$this->url('columns')."'>Check it out!</a> </li>
					<li> <em>Public Groups</em> Want to see what Groups other users have created? Check out the Groups page to view public group timelines. <a class='ignore' href='".$this->url('groups')."'>Check it out!</a></li>
					<li> <em>Timeline Search</em> No more trolling through your timeline for old tweets. Use the 'Timeline Search' at the top of every page to search just your timeline! You can also use advanced search options like: 'from:<user>', 'to:<user>', 'to:me', 'has:link', 'tag:<tag>'. </li>
				</ul>
			</div>
		";
	
		$this->query(" UPDATE users SET bits = '1' WHERE id = '??' ",array($this->uid));
