...
<div class="yui-gc">
	<div class="yui-u first">
		...
		<?php if ( $this->loged ) { ?>
		<div class='fav-group'>
			<a href='#' class='do-fav-group bubble' title="Mark as a Favorite Group" id='g|<?php echo $g['id']; ?>|<?php echo $this->md5($this->uid.$g['id']); ?>'><?php echo (isset($this->user['favGroups'][$g['id']])?'My Favorite':'Favorite'); ?></a>
		</div>
		<?php } ?>
	...
	<div class="yui-u side">
			...
			<?php
				...
				if ( $friends ) {
					...
					foreach ( $users as $u ) {
						if ( $friends AND array_key_exists($u['friend_id'],$friends) ) {
