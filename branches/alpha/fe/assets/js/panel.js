YUI.add("tt-panel",function(Y) {

	// shortcuts
	var $ = Y.get, $j = Y.JSON;

	// base 
	T.Class.Panel = function(args) {
		this.init(args);
	}

	// base prototype
	T.Class.Panel.prototype = {
		...
		load : function(url,args) {
			...
			// url
			var url = T.Obj.getUrl(url,{'.context':'xhr'});			
					...
					//	window.location.href = reg_url;
				 	...
				 	'complete': function(id,o,a) {
						...
						this.obj.get('boundingBox').all('form').each(function(el){						
							...
							// reset it 
							el.setAttribute('x-action', T.Obj.getUrl(action,{'.context':'xhr'}));
	...
	Y.augment(T.Class.Panel, Y.EventTarget);
