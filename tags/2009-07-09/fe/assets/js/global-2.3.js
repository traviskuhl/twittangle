	...
	$Y.twitTangle.Global = function(p) {
		...
		$Y.twitTangle.Global.prototype = {
			...
			init : function(p) {			
				...
				$e.on('strip','mouseover',this.mouseover,this,true);
				$e.on('strip','mouseout',this.mouseout,this,true);
			...
			mouseover : function(e) {
				...
				else if ( ( tar = $d.isGoodTarget(oTar,'search',10) ) && tar.tagName.toLowerCase() == 'form' ) {
			...
			mouseout : function(e) {
				...
				else if ( ( tar = $d.isGoodTarget(tar,'search',20) ) && tar.tagName.toLowerCase() == 'form' ) {
				...
				this.flags.searchSugg = window.setTimeout(function(){
					...
					var callback = {
						'success': function(o) {
							...
							// just add
							$('search-other').innerHTML = j.resp.html;
							
							// defer
							$d.removeClass( $d.getElementsByClassName('defer','img',$('search-other')), 'defer' );
					...
					// url
					var url = self.xhrUrl('searchSuggest',{'q':q});
				...
				this.flags.timelineTo = window.setInterval(function(){
					...
					var callback = {
						'success': function(o) {
							...
							// max 
							self.flags.timelineUpdateData.max = j.resp.max;
			...
			submitUserPanel : function(tar) {
				...
				var data = [];
					...
					data[data.length] = 'tags='+textarea[0].value;
				...
				var callback = {
					'success': function(o) {
						...
						// reset button
						var btn = $('user-panel').getElementsByTagName('button')[0];
			...
			loadXhrPage : function(tar,e) {		
				...
				if ( page.indexOf('home/timeline') != -1 || page == 'home' ) {
				
					// define holder
					var holder = document.createElement('div');
					holder.innerHTML = "<div class='timeline-loading'><em>Loading Your Timeline</em>" +
					...
					if ( $d.inDocument('timeline') ) {													  
						...
						$('page-content').appendChild(holder);
					}
