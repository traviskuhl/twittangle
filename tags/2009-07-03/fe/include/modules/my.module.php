...
class my extends Tangle {
	...
	public function _import() {
		...
		// how long 
		if ( $this->user['friends_count'] > 0 AND time()-$this->user['friends_updated'] < (60*60) ) { 
			...
			if ( $this->pathParam(1) == 'do' ) {
				...
				// get them
				while ( count($data) == 100 AND $i < MAX_PAGES ) {
				...
				if ( trim($r) == 'true' ) {
					...
					// update
					$this->query("UPDATE users SET friends_updated = '??', friends_count = '??' WHERE id = '??' ",array(time(),count($friends),$this->uid));	
