...
*/		
		...
		public function buildTimeline() {
			...
			foreach ( $tweets as $item ) {
				...
				$this->query("
						INSERT INTO {$tbl} 
						SET `id` = '??', `user` = '??', `name` = '??', `text` = '??', `groups` = '??', `rating` = '??', `tags` = '??', `timestamp` = '??', `data` = '??' ON DUPLICATE KEY UPDATE `viewed` = UNIX_TIMESTAMP()
					",array(
					$item['id'],
					$item['user']['id'],
	 ...
	 ************************************/
	 	...
	 	public function _bit_displayNetworkPost($row,$showNetwork=true) {
			...
			$html .= "
					...
					<div class='ft'>
						<span>".$this->displayTimestamp( $row['timestamp'] )." 
			";
...
*/
		...
		public function _bit_displayStatus($row,$live=false,$pic=false,$defer=true) {
			...
			$html .= "
					...
					<div class='ft'>
						<span>".$this->displayTimestamp( strtotime($row["created_at"]) )." from ";
						
			if ( array_key_exists('source',$row) AND strpos($row['source'],'twitTangle') !== false AND ( $net = $this->isNetworkPost($row['id']) ) ) {
				$html .= " network <a href='".$this->url('network',array('slug'=>$net[1]))."'>{$net[0]}</a>";
			}
			else if (array_key_exists('source',$row)) {
				$html .= str_replace("<a ","<a class='ext-link-catch' ",$row['source']);
			}
			
				// reply
				if ( array_key_exists('in_reply_to_status_id',$row) AND $row['in_reply_to_status_id'] ) {
					$html .= " <a class='show-reply' id='reply|{$row['in_reply_to_status_id']}' target='_blank' href='http://twitter.com/{$row['in_reply_to_screen_name']}/status/{$row['in_reply_to_status_id']}'>in reply to {$row['in_reply_to_screen_name']}</a>";
				}
				...
				if ( !$live AND $this->loged ) {
						...
						$html .= "
							<span class='usr'>
								<a href='#reply' class='reply' id='reply|{$row['id']}'>Reply</a> &nbsp;
								<a href='#retweet' class='retweet' id='retweet|{$row['id']}'>Re-Tweet</a> &nbsp;
								".( (array_key_exists('favorited',$row) AND $row['favorited'])?'<em class="b">Favorite</em>':"<a href='#fav' class='fav' id='fav|{$row['id']}'>Favorite</a>")."
							</span>						
						";
