<?php
	...
	require_once("/usr/share/pear/Pager/Pager.php");	
	require_once("/usr/share/pear/Pager/Sliding.php");
	require_once("/usr/share/pear/Crypt/Blowfish.php");
	
	
		// simple functions 
		function _possessive($str) { return ($str{strlen($str)-1} == 's'?$str."'":$str."'s"); }
	...
	class Base extends Config {
						...
						if ( $val ) {
							switch($key) {									
								case 'profile_background_image_url':
									$this->profileCss .= " body { background-image: url($val); background-repeat: no-repeat; background-attachment: fixed; } body #doc4 { background:none; } body #doc4 #hd .wrap { background-color: transparent; } "; 
									break;
		...
		public function xhrTemplate($body) {
			...
			exit( json_encode( array( 
				...
				'c' => $this->bodyClass,
				't' => html_entity_decode($this->title . ' on twitTangle | untangling the mess of too many friends ',ENT_QUOTES,"utf-8"),
		...
		public function query( $sql, $params=0, &$pagerArgs=null ) {
			...
			// die
			if ( $this->dbh->error ) {
				@file_put_contents("/home/twitter/tmp/cache/db_errors","[ERROR] ".$this->dbh->error."\n   [SQL] ".preg_replace("/(\t|\n)+/","",$sql)."\n",FILE_APPEND);
			}
		...
		public function saveCache($key,$data,$ttl=0,$type='fe') {		
			...
			if ( !file_exists( $this->config['dir']['cache'] . $type . '/' . $sub ) ) {
				mkdir( $this->config['dir']['cache'] . $type . '/' . $sub );
			}		
		...
		public function encrypt($str) {
			...
			/* blogfish */
			$bf = new Crypt_Blowfish($this->config['key']);		
		...
		public function decrypt($str) {
			...
			// do it
			return str_replace("\0","",$bf->decrypt($str));
