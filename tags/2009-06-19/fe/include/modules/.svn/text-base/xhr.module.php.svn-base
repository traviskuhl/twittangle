...
*/
	...
	public function timelineUpdate() {		
		...
		$html = "<ul>";
			...
			switch($type) {
				...
				case 'my-groups':
					...
					// get my timeline
					$r = $this->twitter("statuses/friends_timeline.json?since_id={$since}&count=200",false,true);
					...
					if ( is_array($r) AND count($r) > 0 ) {
						...
						foreach ( array_reverse($r) as $i ) {
							if ( array_key_exists($i['user']['id'],$users) ) {					
		...
		// we're all good
		$this->good(array('html'=>$html,'bootstrap'=>$bs,'raw'=>$raw,'max'=>$max));
