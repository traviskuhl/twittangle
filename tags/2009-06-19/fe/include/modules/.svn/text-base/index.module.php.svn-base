...
class index extends Tangle {
	...
	public function home() {
		...
		$loadnow = true;
			...
			// if alpha
			if ( $this->alpha AND $this->param('_ctx') != 'xhr' ) {
				$loadnow = false;
			}
		...
		if ( $loadnow ) {	
			...
			if ( $path[0] == 'timeline' ) {
				...
				// title
				$title = "What Your Friends Are Doing";
			
				// get them 
				list($h,$timeline,$pages) = $this->getTweets($menu_timeline[$path[1]][1],$page,false,20,false,false);			
				...
				// get them 
				list($h,$timeline,$pages) = $this->getTweets('yesterday',$page,false,20,false,$path[1]);					
