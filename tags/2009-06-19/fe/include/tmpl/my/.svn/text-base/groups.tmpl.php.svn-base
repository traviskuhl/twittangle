			...
			<?php 
				...
				if ( count($groups) == 0 ) {
					...
					foreach ( $groups as $g ) {
						...
						// title
						echo "
							<li id='group-{$g['id']}' class='group-top ".($i++==0?'open':'')." wrap wrap-square'> 
						...
						<?php
							...
							foreach ( $thisGroupUsers as $u ) {
								...
								if ( array_key_exists($u['friend_id'],$this->allFriends) ) {
									...
									echo "
										<li>
											<img class='defer-pic user-panel' id='up|{$usr['id']}|{$usr['sn']}' src='".BLANK."' style='background-image: url({$pic});'>
...
<script type="text/javascript">
	TT.addToQueue(function(){
		...
		// get
		YAHOO.util.Get.css("<?php echo $this->asset('css','groups','1.2'); ?>",{
			'onSuccess': function() {
