...
var Element = function(type) { 
	...
	Element.prototype = {
		...
		addClass:function(cl) {		
			...
			for (var c in cl ) {
				$d.addClass(this.el,cl[c]);
			}
			return this;
		},
		removeClass:function(cl) {
			$d.removeClass(this.el,cl);
			return this;			
		},
		appendChild:function(ch) {			
			if ( typeof ch.isElement != 'undefined' ) {
				ch = ch.get();
			}
	...
	$Y.twitTangle.Global = function(p) {
		...
		$Y.twitTangle.Global.prototype = {
			...
			click : function(e) {	
				...
				if ( $d.hasClass(tar,'user-panel-submit') ) {
				...
				else if ( $d.hasClass(tar,'do-fav-group') ) {
					...
					var parent = $d.isGoodTarget(tar,'fav-group');
					...
					var callback = {
						'success': function(o) {  
							...
							if ( j.resp ) {
								o.argument[0].innerHTML = "My Favorite";
							}
							else {
								o.argument[0].innerHTML = "Favorite";
							}
						},
						'argument': [parent]
					}
					...
					// go
					var url = this.xhrUrl('favgroup',{'id':info[1],'token':info[2]});
													
					// send
					var r = $c.asyncRequest('GET',url,callback);				
				
				}
				else if ( $d.hasClass(tar,'unhide') ) {
					...
					// do it
					$d.removeClass( $d.getElementsByClassName('hide','*', $(info[1]) ), 'hide' );
				
					// remove el
					var li = $d.isGoodTarget(tar,'more');
					
					li.parentNode.removeChild(li);
				
				}
				else if ( $d.hasClass(tar,'update-group') ) {
					...
					var callback = {
						'success': function(o) {
							...
							// what 
							if ( j.stat != 1 ) { alert(j.msg); }

							// we're all good close 
							o.argument[0].flags.editGroupPanel.close();
					 	
					 		// reload
							o.argument[0].loadXhrPage({'href':"/my/groups/"+j.resp.id});
					...
					// url 
					var url = this.xhrUrl('updategroup');
					
					// form
					YAHOO.util.Connect.setForm( this.flags.editGroupPanel.cnt.getElementsByTagName('form')[0] );
					
					//go 
					var r = $c.asyncRequest('POST',url,callback);
				
				}
				else if ( $d.hasClass(tar,'edit-group') ) {
					...
					// no panel yet
					if ( !this.flags.editGroupPanel ) {
						this.flags.editGroupPanel = new $Y.twitTangle.Panel({
							'name': 'editgroup',
							'overlay': true,
							'noOpen': true
						});					
					...
					// load
					this.flags.editGroupPanel.load({
						'url': this.xhrUrl('editgroup',{'id':info[1]}),
						'open': true
					});				
				...
				else if ( $d.hasClass(tar,'update-status-dm') ) {
				
					// stop
					$e.stopEvent(e); tar.blur;				
					
					// title
					$('update-status-title').innerHTML = "Send a Direct Message <a class='gray clear-status' href='#'>clear</a>";					
					
					// set text
					$('update-status-txt').value = "dm @";
...
*/
				...
				else if ( $d.hasClass(oTar,'remove-user-group') ) {
				
					// stop 
					$e.stopEvent(e);				
				
					// get it 
					var info = oTar.id.split('|');
					
					// find the li 
					var li = $d.isGoodTarget(oTar,'group-user-top');
					
					// remove
					li.parentNode.removeChild(li);
					
					// do it
					var url = this.xhrUrl('removeUserFromGroup',{'uid':info[1],'gid':info[2]});
					
					// go 
					var c = $c.asyncRequest('GET',url);
				...
				else if ( this.params.loged && !this.disableAjaxLoad && !$d.hasClass(oTar,'ignore') && ( tar = $d.isGoodTarget(oTar,{'tag':'a'}) ) ) {
			...
			mouseover : function(e) {
				...
				// what 
				if ( ( tar = $d.isGoodTarget(oTar,'status') ) ) {
					
					this.titleBubble(oTar,e,'on');
					
					// turn it on
					$d.addClass(tar,'on');
				
				}				
				else if ( $d.hasClass(oTar,'bubble') ) {
					this.titleBubble(oTar,e,'on');
				}				
			...
			mouseout : function(e) {
				...
				if ( $d.hasClass(tar,'bubble') ) {
					...
					$d.removeClass(tar,'focused');					
					
				}				
				else if ( ( tar = $d.isGoodTarget(oTar,'status') ) ) {
					
					this.titleBubble(oTar,e,'out');
			...
			displayUserPanelOverlay : function(tar) {
				...
				if ( !this.params.loged ) {
					window.location.href = tar.getAttribute('href'); return;
				}				
			...
			/* showSearchExample */
			showSearchExample : function(str) {
			
				// searcj
				$('search-txt').value = str;
				
				// hide
				this.toggleSearchExamples(0);
			...
			/* hidesearchexamples */
			toggleSearchExamples: function(s) {
			
				// att
				var s = s;			
				var at = {'opacity':{to:(s?1:0)}};
				
				var a = new $a('saved-examples',at,.2);
					a.onComplete.subscribe(function(){
						$d.setStyle('saved-examples','display',(s?'block':'none'));
					});
					a.animate();
			...
			searchSuggest : function(e) {
				...
				this.flags.searchSugg = window.setTimeout(function(){
					...
					// make bigger
					var a = new $a('search-result',{'height':{ 'to': 150 } },.2);
						a.onComplete.subscribe(function(){
							$d.setStyle('search-result','height','auto');
						});
						a.animate();
			...
			loadAndOpen : function(uri,params,holder) {
				...
				// define a cache id 
				var cid = md5( $j.stringify([uri,params]) );		
				...
				var callback = {
					'success': function(o) {
						...
						// save
						o.argument[0].cache[o.argument[2]] = o;
					
					},
					'argument': [this,holder,cid]
				};
				
				// check for cache 
				if ( this.cache[cid] ) {
					callback.success(this.cache[cid]); return;	
				}				
					...
					var callback = {
						'success': function(o) {
							...
							var items = $('payload').getElementsByTagName('li');
								...
								// remove defer
								$d.removeClass('defer', $d.getElementsByClassName('defer','*', $('timeline') ) );															
							...
							// groups
							if ( j.resp.groups ) {
								for ( var g in j.resp.groups ) {
									if ( j.resp.groups[g] > 0 ) {
									
										// obj
										var obj = $('group-count-'+g);
										...
										// span
										span.innerHTML = parseInt(j.resp.groups[g],10) + parseInt(span.innerHTML,10);
			...
			doReply : function(tar) {
				...
				if ( !$d.inDocument('update-status')  ) {
					...
					// create a new panel
					this.flags.doreplypanel = new $Y.twitTangle.Panel({
						'name': 'doreply',
						'overlay': true,
						'noOpen': true,
						'noRender': true
					...
					// lfags
					this.flags.doreplypanel.height = 130;
					
					//
					this.flags.doreplypanel.render();
					
					$d.setStyle(this.flags.doreplypanel.cnt,'min-height','130px');
				
					// load
					this.flags.doreplypanel.load({
						'url': this.xhrUrl('updatebox'),
						'open': true,
						'onLoad': function() {
							$d.setStyle(this.cnt,'height','200px');
							self.doReply(target);							
						}
					});	
					
					this.flags.doreplypanel.onClose.subscribe(function(){
						self.flags.doreplypanel.destroy();
						self.flags.doreplypanel = false;
					});
			...
			addUserToGroup : function(group,args) {
				...
				var callback = {
					'success': function(o) {
						...
						li.className = 'group-user-top';								
						li.innerHTML = "<a id='up|"+a.id+"' class='user-panel' href='/user/"+a.sn+"'><img src='"+a.img+"'></a>" +
									   "<a href='/user/"+a.sn+"'>"+a.name+"</a>" +
									   "<em>"+a.sn+"</em>";
			...
			submitUpdate : function(tar) {
				...
				var callback = {
					'success': function(o) {
						...
						// what 
						if ( o.argument[0].flags.doreplypanel ) {
							o.argument[0].flags.doreplypanel.close();
						}						
			...
			submitUserPanel : function(tar) {
				...
				var textarea = $('userpanel-content').getElementsByTagName('textarea')[0]; 
		
				// data	
				var data = [];
					
					// add rate
					if ($d.inDocument('user-panel-rate-new')) {
						data[data.length] = 'rate='+$('user-panel-rate-new').value;
					}
					else {
					...
					// tags
					data[data.length] = "tags="+textarea.value;
	...
	$Y.twitTangle.Panel = function(p) {
		...
		$Y.twitTangle.Panel.prototype = {
			...
			render : function() {
				...
				// inner
				wrap.innerHTML = "<div class='panel-bd'><div class='panel-cnt' id='"+p.name+"-content'></div></div>  <span class='bg'></span> <a class='panel-close' href='#'>close</a>";
			...
			open : function(p) {
               ...
               var reg = $d.getRegion(this.cnt);
               
                // center
                var pw = reg.right-reg.left;
                var ph = reg.bottom-reg.top;
                ...
                // x
                var x = (dw/2) - (pw/2);
                var y = parseInt($d.getDocumentScrollTop(),10) + ((dh/2) - (ph/2));
				...
				// fade in
				if ( $Y.env.ua.ie > 0 ) {
    				$d.setStyle(this.wrap,'opacity',1);
				}
				else {				
