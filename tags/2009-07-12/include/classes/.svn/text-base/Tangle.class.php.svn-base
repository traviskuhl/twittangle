		...
		public function buildTimeline() {
			...
			foreach ( $tweets as $item ) {
				...
				$this->query("
						...
						SET `for_user` = '??', `id` = '??', `user` = '??', `name` = '??', `text` = '??', `groups` = '??', `rating` = '??', `tags` = '??', `timestamp` = '??', `data` = '??'
					",array(
					$this->uid,
					$item['id'],
					$item['user']['id'],
...
*/
		...
		public function _bit_displayStatus($row,$live=false,$pic=false,$defer=true) {
			...
			if ( strpos($row['source'],'twitTangle') !== false AND ( $net = $this->isNetworkPost($row['id']) ) ) {
